...
const WorkoutModal = ({ workout, onClose, onUpdate }: WorkoutModalProps) => {
  ...
  const [notes, setNotes] = useState(workout.user_notes || "");
    ...
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 bg-gradient-to-b from-slate-900 to-slate-950 rounded-t-3xl animate-slide-up max-h-[85vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4 text-white">
          ...
          {workout.description && !workout.warmup && !workout.main_set && !workout.cooldown && (
          ...
          {/* Metrics - Compact */}
          {((workout.duration_minutes !== undefined && workout.duration_minutes !== null && workout.duration_minutes > 0) || 
            (workout.distance_km !== undefined && workout.distance_km !== null && workout.distance_km > 0)) && (
            <div className="flex gap-3">
              {workout.duration_minutes !== undefined && workout.duration_minutes !== null && workout.duration_minutes > 0 && (
                <div className="flex-1 bg-white/5 rounded-lg p-3 flex items-center gap-2">
                  <Clock className="w-5 h-5 text-primary flex-shrink-0" />
                  <div>
                    <div className="text-lg font-bold">{workout.duration_minutes}</div>
              ...
              {workout.distance_km !== undefined && workout.distance_km !== null && workout.distance_km > 0 && (
                <div className="flex-1 bg-white/5 rounded-lg p-3 flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-lg font-bold">{workout.distance_km}</div>
