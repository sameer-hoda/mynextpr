# Step-by-Step Migration Guide: Supabase to SQLite

## Phase 1: Preparation and Environment Setup

### Step 1: Backup Current Data (Optional but Recommended)

Before starting the migration, you may want to backup your existing Supabase data:

```bash
# If you have access to your Supabase database, export the current data
# This is for reference and potential data migration later
# Create a backup of your current application state
git add .
...
Run these commands in your `/Users/sameerhoda/alt_projects/runna/mynextpr_v2/plan-my-run` directory:

```bash
# Install SQLite dependencies for browser-based operation
npm install sql.js

# Install TypeScript definitions
npm install --save-dev @types/sql.js

# We'll use browser-based solutions, so no need for node-sqlite3
# For IndexedDB storage, install additional utilities
npm install idb-keyval
```

### Step 3: Create Directory Structure

Create the necessary directories for the new modules:

```bash
mkdir -p src/integrations/sqlite
mkdir -p src/integrations/auth
mkdir -p src/utils
```

## Phase 2: Create the SQLite Wrapper Module

### Step 4: Create SQLite Wrapper

Create the file: `src/integrations/sqlite/wrapper.ts`

```typescript
// src/integrations/sqlite/wrapper.ts
import initSqlJs, { Database } from 'sql.js';

// Define TypeScript interfaces matching your Supabase tables
interface Profile {
  id: string;
  user_id: string;
...
class SQLiteWrapper {
  ...
  private createTables() {
    ...
    this.db.run("CREATE INDEX IF NOT EXISTS idx_workouts_completed ON workouts(completed);");
    this.db.run("CREATE INDEX IF NOT EXISTS idx_plans_user_id ON plans(user_id);");
    
    console.debug("DEBUG: All SQLite tables and indexes created successfully");
  }
    ...
    try {
      ...
      if (profile) {
        ...
        this.db.run(`
          ...
          VALUES (?, ?, ?, ?, ?)
        `, [profileId, userId, displayName, now, now]);
  ...
  async getPlans(userId: string) {
    ...
    try {
      ...
      console.debug("DEBUG: Plans fetched:", result.length, "plans found");
      return result;
    } catch (error) {
  ...
  async getWorkouts(userId: string) {
    ...
    try {
      ...
      console.debug("DEBUG: Workouts fetched:", result.length, "workouts found");
      return result;
    } catch (error) {
  ...
  async getCompletedWorkouts(userId: string) {
    ...
    try {
      ...
      console.debug("DEBUG: Completed workouts fetched:", result.length, "completed workouts found");
      return result;
    } catch (error) {
...
## Phase 3: Create the Local Authentication Service

### Step 5: Create Local Authentication Service

Create the file: `src/integrations/auth/localAuth.ts`
...
## Phase 4: Update the Authentication Context

### Step 6: Update AuthContext to Use Local Auth
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      const mockUser = result.data?.user;
      if (mockUser) {
        ...
        await sqliteDB.upsertProfile(mockUser.id, displayName);
      }
...
## Phase 5: Update the Auth Page

### Step 7: Update Auth.tsx to Use Local Authentication

Replace the content of `src/pages/Auth.tsx`:
...
## Phase 6: Update Plan Page

### Step 8: Update Plan.tsx to Use SQLite

Replace the content of `src/pages/Plan.tsx`:
...
## Phase 7: Update Journal Page

### Step 9: Update Journal.tsx to Use SQLite

Replace the content of `src/pages/Journal.tsx`:
...
### Step 10: Update Vite Configuration

First, update your `vite.config.ts` to handle the SQL.js WASM file if needed:

```typescript
// In vite.config.ts, you might need to add:
...
export default defineConfig({
  ...
  // If you encounter issues with WASM loading, you may need this:
  optimizeDeps: {
    include: ['sql.js'],
  }
})
...
```bash
cd /Users/sameerhoda/alt_projects/runna/mynextpr_v2/plan-my-run
npm install sql.js @types/sql.js idb-keyval
```
...
3. **Test the application:**
   - Try logging in with both Google and mock user
   - Verify that you can navigate to Plan and Journal pages
   - Check that data persists between page refreshes
   - Verify that workouts are displayed correctly

### Step 12: Additional Component Updates

You may need to update other pages as well. For example, if there are any other pages that use Supabase directly, they would need to be updated to use the SQLite wrapper instead.

## Important Notes:

1. **Current Limitations**: This approach stores data in memory during the session. For persistent storage across browser sessions, you'd need to add serialization to localStorage or IndexedDB.

2. **Google OAuth**: The local auth implementation simulates Google login. For a real production implementation, you'd need to integrate with Google's OAuth API separately from the database.

3. **Data Migration**: If you have existing Supabase data you want to migrate, you'll need to add a migration function that exports your current data and imports it into the SQLite database.

4. **Error Handling**: The debugging logs are comprehensive and will help you identify any issues during the migration.

This migration approach maintains all functionality while replacing the Supabase backend with an in-browser SQLite database.